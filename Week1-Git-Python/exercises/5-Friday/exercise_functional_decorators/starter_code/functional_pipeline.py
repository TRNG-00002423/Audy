...
fail_first = sorted(test_results, key = lambda x: (x["status"] != "fail", x["name"]))

for x in by_duration:
...
critical = list(filter(lambda x: x["status"] == "fail" and x["duration_ms"] > 1500, test_results))
...
total_duration = reduce(
    lambda acc, x: acc + x["duration_ms"],
    test_results,
    0
)
...
total_fail_duration = reduce(
    lambda acc, x: acc + x["duration_ms"],
    filter(
        lambda x: x["status"] == "fail", test_results
        ),
        0 # initial value
)

# TODO 4c: Find the name of the test with the LONGEST name (by character count).
# TODO — reduce() over names
longest_name = reduce(
...
# TODO — reduce() with a dict accumulator and initializer={}
module_counts = reduce(
